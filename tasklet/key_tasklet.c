...
#define KEY_INTERRUPT_NAME		"keyTaskletBingo"
			 						...
			 						size_t len, loff_t *ppos)
{
	...
	{
		atomic_set(&dev->readKeyIndex,writeData);
	}	
	return 0;
}
				  ...
				  size_t count, loff_t *ppos)
{
	...
	if(atomic_read(&dev->key[atomic_read(&dev->readKeyIndex)].pressFlag) == KEY_VALUE_RAISE)
	{
		value = 1;
		atomic_set(&dev->key[atomic_read(&dev->readKeyIndex)].pressFlag,KEY_VALUE_NONE);
	}
	else
	{
		value = 0;
...
#endif
	/* 调度tasklet */
	tasklet_schedule(&dev->key[0].taskletTab);

	
	return IRQ_HANDLED;
}

void taskletFunc_key_0(unsigned long data)
{
	struct keyInfo* dev = (struct keyInfo*)data;
	/* 激活定时器进行消抖处理 */
	if(atomic_read(&dev->pressFlag) != KEY_VALUE_RAISE)	//上一次按键按下处理完成后才处理下一次按键
	{
...
static void (*taskletFuncArray[KEY_NUM])(unsigned long) = {taskletFunc_key_0};
...
static int key_init_irq_io(struct keyInterruptInfo * dev)
{
	...
	for(i=0;i<KEY_NUM;i++)
	{
		...
		dev->key[i].taskletFunc = taskletFuncArray[i];
		tasklet_init(&dev->key[i].taskletTab,dev->key[i].taskletFunc, (unsigned long)&dev->key[i]);
