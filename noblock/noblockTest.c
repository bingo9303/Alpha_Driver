				  ...
				  size_t count, loff_t *ppos)
{
	...
	if(file->f_flags & O_NONBLOCK)	//这个标志位置1为非阻塞
	{
		return -EAGAIN;
	}
	else	//标志位没有置位为阻塞
...
static unsigned int key_interrupt_poll(struct file *file, struct poll_table_struct * wait_table)
{
	int i,mask=0;
	struct keyInterruptInfo* dev = (struct keyInterruptInfo*)file->private_data;

	poll_wait(file, &dev->r_wait, wait_table);	//将等待队列头添加到poll_table中，不会引起阻塞
