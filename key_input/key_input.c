...
{
	file->private_data = (struct keyInputInfo*)&keyInputInfo;
	return 0;
}
...
int hasVaildKey(struct keyInputInfo* dev)
{
	int i;
	for(i=0;i<KEY_NUM;i++)
	{
				  ...
				  size_t count, loff_t *ppos)
{
	...
	int i;
	struct keyInputInfo* dev = (struct keyInputInfo*)file->private_data;
#if 0
...
{
	struct keyInputInfo* dev = (struct keyInputInfo*)dev_id;
	...
	if(value == atomic_read(&dev->keyValue))/* 如果按键状态还是相同 */
	{
		...
		else if(value == 1)
		{
			...
			if(atomic_read(&dev->pressFlag) == KEY_VALUE_PRESS)
			{
				...
				wake_up(&keyInputInfo.r_wait);		//唤醒
			}
			else
			{
...
static int key_init_irq_io(struct keyInputInfo * dev)
{
	int i;
	int result = 0;
...
static int __init key_input_init(void)
{
	...
	keyInputInfo.input_key_dev->evbit[0] = 
